...
function MJ_iniciar(q){
	...
	if(!q)setTimeout("Posicion(0);",100);
...
function MJ_simple(titulo,mensaje){
	...
	if(val("dispositivo")!="m")document.getElementById('aceptar_aviso').focus();
}
...
function MJ_reTry(f){
	...
	else if(confirm("Parece que no tiene conexion a internet. Desea intentar nuevamente?"))eval(f+";");
}
...
function val(id, v) {
	if (v == null || v == undefined) return $("#" + id).val();
	else $("#" + id).val(v);
}
